...
export default function HoverPlayer() {
  const parsedElements = useMemo(() => getTopLevelReadableElementsOnPage(), []);

  const result = useHoveredParagraphCoordinate(parsedElements);

  if (!result) return null;

  const { element, heightOfFirstLine, left, top } = result;
  ...
  return (
    ...
    >
      <PlayButton onClick={() => speechify(element)} />
    </div>
  );
}
